...
static void ColouriseTuringDoc(
	...
	for (; sc.More(); sc.Forward()) {
		...
		} else if (sc.state == SCE_LUA_COMMENT) {
			if (sc.Match('/', '*')) {
					nestLevel++;
			} else if (sc.Match('*', '/')) {
					nestLevel--;
	...
	for (unsigned int i = startPos; i < lengthDoc; i++) {
		...
		if (style == SCE_LUA_WORD) {
				...
				) 
				{
					...
					// don't trigger an indent
					bool isOnNewLine = true;
					// loop to beginning of array looking for newlines
					if(i > 0){
...
static const char * const turingWordListDesc[] = {
	"Keywords",
	"Basic functions",
...
LexerModule lmTuring(SCLEX_TURING, ColouriseTuringDoc, "turing", FoldTuringDoc, turingWordListDesc, 6);
